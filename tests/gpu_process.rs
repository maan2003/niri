...
fn smoke_in_separate_process() {
    ...
    let mut client = GpuClient::spawn_process(exe, Mode::Headless)
        .expect("spawning gpu process (needs EGL, e.g. llvmpipe)");
    // Everything below runs under the seccomp allowlist, like the real compositor.
    client.lockdown().expect("locking down the gpu process");
