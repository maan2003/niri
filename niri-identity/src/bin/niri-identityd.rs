...
struct Args {
    ...
    config: PathBuf,
    /// Where allocated UIDs are remembered. Defaults to
    /// `$XDG_STATE_HOME/niri-identity/uids.toml`.
    #[arg(long)]
...
fn main() -> ExitCode {
    ...
    let state = args.state.or_else(state_path);
    let Some(state) = state else {
        eprintln!("niri-identityd: --state not given and neither XDG_STATE_HOME nor HOME is set");
        return ExitCode::FAILURE;
    };
    ...
    let identity = match Identity::new(config, state, forker, base_env) {
...
fn state_path() -> Option<PathBuf> {
    ...
    Some(base.join("niri-identity/uids.toml"))
}
