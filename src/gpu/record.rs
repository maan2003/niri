//! Core side of element recording: sends every element of a frame to the GPU process wrapped
//! in element markers, with the damage since the last frame that element was recorded in.
//!
//! One `Recorder` per render target (output or screencast stream), because damage is relative
//! to what that target last saw.
...
use super::convert;
use super::protocol::{ElementKind, ElementMeta, ElementState, Presentation};
...
impl Recorder {
    /// Forgets the element history; the next frame sends full damage for everything.
    pub fn clear(&mut self) {
        self.tracks.clear();
    }

    /// Records `elements` (top to bottom) into `target`, bottom to top, each wrapped in
    /// element markers. `size` is the untransformed buffer size.
