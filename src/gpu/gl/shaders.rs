//! GPU-process side: compiled GL programs, stored in the EGL context user data.

use std::cell::RefCell;

use smithay::backend::renderer::gles::{
    GlesError, GlesFrame, GlesRenderer, GlesTexProgram, UniformName, UniformType,
};
...
pub fn set_custom_resize_program(renderer: &mut GlesRenderer, src: Option<&str>) -> bool {
...
pub fn set_custom_close_program(renderer: &mut GlesRenderer, src: Option<&str>) -> bool {
    let program = if let Some(src) = src {
        match compile_close_program(renderer, src) {
            Ok(program) => Some(program),
            Err(err) => {
                warn!("error compiling custom close shader: {err:?}");
                return Shaders::get(renderer).program(ProgramType::Close).is_some();
            }
        }
    } else {
        None
    ...
    Shaders::get(renderer).program(ProgramType::Close).is_some()
}
...
pub fn set_custom_open_program(renderer: &mut GlesRenderer, src: Option<&str>) -> bool {
    let program = if let Some(src) = src {
        match compile_open_program(renderer, src) {
            Ok(program) => Some(program),
            Err(err) => {
                warn!("error compiling custom open shader: {err:?}");
                return Shaders::get(renderer).program(ProgramType::Open).is_some();
            }
        }
    } else {
        None
    ...
    Shaders::get(renderer).program(ProgramType::Open).is_some()
}
