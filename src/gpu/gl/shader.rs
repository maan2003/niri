//! GPU-process side: niri's custom pixel-shader programs and the draw routine for them.
...
/// Everything the core recorded for one shader draw.
pub struct DrawParams<'a> {
    pub src: Rectangle<f64, Buffer>,
    pub dest: Rectangle<i32, Physical>,
    pub damage: &'a [Rectangle<i32, Physical>],
    pub scale: f32,
    pub alpha: f32,
    pub uniforms: &'a [Uniform<'a>],
    pub textures: &'a [(String, GlesTexture)],
}
...
pub fn draw(
    ...
    let _span = tracy_client::span!("gl::shader::draw");
    ...
    let clamp = |rect: &Rectangle<i32, Physical>| {
    ...
    if supports_instancing {
        resources.vertices.extend(damage.iter().flat_map(clamp));
    } else {
        resources.vertices.extend(
            damage
                .iter()
                .flat_map(|rect| (0..6).flat_map(move |_| clamp(rect))),
        );
    }
    ...
    frame.with_profiled_context(span_loc, move |gl| -> Result<(), GlesError> {
        ...
        unsafe {
            for (i, (_, texture)) in params.textures.iter().enumerate() {
            ...
            for (i, (name, _)) in params.textures.iter().enumerate() {
                let Some(location) = program.texture_uniforms.get(name) else {
                    return Err(GlesError::UnknownUniform(name.clone()));
                };
                gl.Uniform1i(*location, i as i32);
            }
            gl.UniformMatrix3fv(
            ...
            gl.Uniform1f(program.uniform_scale, params.scale);
            gl.Uniform1f(program.uniform_alpha, params.alpha);
            ...
            gl.BufferData(
                ffi::ARRAY_BUFFER,
                (std::mem::size_of::<ffi::types::GLfloat>() * resources.vertices.len()) as isize,
